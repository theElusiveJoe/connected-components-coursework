...
func (slave *slaveNode) log(format string, args ...any) {
	msg := fmt.Sprintf("%d ", time.Now().UnixMilli())
	msg += fmt.Sprintf("%d ", slave.rank)
	msg += fmt.Sprintf("%d ", slave.curIter)
	msg += fmt.Sprintf("%d ", slave.curStep)
...
func (slave *slaveNode) getEdgeAndServers(i int) (uint32, uint32, uint32, uint32) {
	return slave.edges1[i], slave.edges2[i], slave.getServer(slave.edges1[i]), slave.getServer(slave.edges2[i])
}
...
func (slave *slaveNode) isServerOf(v uint32) bool {
	return slave.rank == int(slave.getServer(v))
