...
func (master *masterNode) init() {
	// master.nodesNum = g.NodesNum
	// master.edgesNum = g.Len()
	// master.edges1, master.edges2 = g.Edges1, g.Edges2
	...
	// iter.Init(g)
	// iter.StartIter()
	// master.distribution = distribution.FindDistribution(
	// 	iter, uint32(master.slavesNum), uint32(master.hashNum),
	// )
